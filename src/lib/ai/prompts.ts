...
export const CUSTOMER_REVIEW_INSIGHTS_YELP_PROMPT = ({
    ...
    `
Summarize what customer reviews for businesses in the ${business} market
in and around ${area} (near ${location}) say about "${query}".

Do not list specific locations or business names.
`.trim();
...
export const OFFERING_INSIGHTS_YELP_PROMPT = ({
    ...
    `
Summarize service offering insights for ${business} in and around ${area} (near ${location}),
focusing on how well services meet expectations and perceived value for money.

Look at recent reviews and produce one cohesive, plain-English paragraph that combines:
1) themes where customers want more from the experience (missing services, lack of personalization or atmosphere), and
